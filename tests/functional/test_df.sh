#!/bin/bash
# Functional test: boot miniOS and verify `df` works (statvfs syscall).
set -euo pipefail
...
echo "=== miniOS df (statvfs) Functional Test ==="

QEMU_OUT="$(mktemp /tmp/qemu_df_XXXXXX)"
QEMU_MON="/tmp/qemu_df_mon_$$"
...
if ! wait_for "Filesystem"; then
    echo "FAIL: df header not found in output"
    cat "$QEMU_OUT"
    exit 1
fi

if grep -q "ENOSYS\|Operation not implemented\|task.*exit.*code 1" "$QEMU_OUT"; then
    echo "FAIL: df returned ENOSYS or exited with error"
    grep -E "ENOSYS|not implemented|df:" "$QEMU_OUT" || true
    exit 1
fi

echo "PASS: df ran without ENOSYS"
echo "--- df output ---"
grep -A10 "Filesystem" "$QEMU_OUT" | head -15
